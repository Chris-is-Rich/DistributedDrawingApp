...
public class Draw extends ReceiverAdapter implements ActionListener,
    ...
    public Draw(String props, boolean no_channel, boolean jmx,
            ...
            String name, boolean send_own_state_on_merge,
            AddressGenerator gen) throws Exception {
        ...
        channel = new JChannel(props).addAddressGenerator(gen)
                .setName(name);
    ...
    public Draw(JChannel channel, boolean use_state, long state_timeout)
            throws Exception {
        this.channel = channel;
        channel.setReceiver(this);
        channel.addChannelListener(this);
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if ("-uuid".equals(args[i])) {
                generator = new OneTimeAddressGenerator(
                        Long.valueOf(args[++i]));
                continue;
            }
        ...
        try {
            draw = new Draw(props, no_channel, jmx, use_state,
                    state_timeout, use_unicasts, name,
    ...
    public void go() throws Exception {
      ...
      //  mainFrame.getContentPane().add(label,BorderLayout.CENTER);
        ...
        browse_button.setFont(default_font);
        browse_button.addActionListener(this);
        JButton recieve_button = new JButton("Recieve");
        recieve_button.addActionListener(this); 
        JButton send_button = new JButton("Send");
        send_button.setFont(default_font);
        ...
        sub_panel.add("South", recieve_button);
        sub_panel.add("South", browse_button);
        sub_panel.add("South", send_button);
        ...
        mainFrame.setBounds(new Rectangle(650, 650));
    ...
    public void receive(Message msg) {
        ...
        if (buf == null) {
            System.err.printf(
                    "%s: received null buffer from %s, headers: %s\n",
                    channel.getAddress(), msg.src(), msg.printHeaders());
            return;
        }

        try {
            DrawCommand comm = Util.streamableFromByteBuffer(
                    DrawCommand.class, buf, msg.getOffset(),
                    msg.getLength());
            switch (comm.mode) {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        case "Send":
        	if(image2 != null) {
        		...
        		System.out.println("Error null value set for picture");
        	}
        		break;
        case "Recieve":
        		try {
    ...
    public void MyfileChooser() throws Exception {
    ...
    public void channelDisconnected(JChannel channel) {
        if (jmx) {
            ...
            if (server != null) {
                try {
                    JmxConfigurator.unregisterChannel(channel, server,
                            cluster_name);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected class DrawPanel extends JPanel implements MouseMotionListener {
        ...
        public void drawState() {
            ...
            Color col;
            synchronized (state) {
                for (Iterator it = state.entrySet().iterator(); it
                        .hasNext();) {
